...
import ai.deepright.llm.provider.anthropic.CustomerAnthropicReader;
...
import ai.open.right.workflow.flow.llm.provider.minimax.MiniMaxRouter;
...
@Getter
@Setter
public class CustomerMiniMaxRouter extends MiniMaxRouter {
    ...
    public static class CustomerInitConfig extends InitConfig {
        ...
        @Override
        @Bean(MiniMaxRouter.NAME)
        @ConditionalOnMissingBean(name = MiniMaxRouter.NAME)
        public MiniMaxRouter miniMaxRouter() {
            CustomerMiniMaxRouter miniMaxRouter = new CustomerMiniMaxRouter();
            BeanUtils.copyProperties(this, miniMaxRouter);
            log.info("CustomerMiniMaxModelRouter inited");
