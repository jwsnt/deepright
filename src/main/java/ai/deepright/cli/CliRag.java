...
public class CliRag extends RagCondition implements RagService {
    ...
    protected void updateDir(RagConfig ragConfig, RagData ragData) throws Exception {
        ...
        if (ragData.getQuery().isEntry()) {
            ragData.getQuery().putMetadata(FeatureField.KEY_PLUGIN_DIR, StringUtils.defaultIfEmpty(Paths.get(StringUtils.defaultIfEmpty(FeatureUtils.buildApp(ragData.getQuery()), "")).getParent().toString(), ""));
        }
        String replace = MapUtils.getString(ragConfig.getGlobalConfig(), FeatureField.KEY_PLUGIN_DIR);
        if (!StringUtils.isEmpty(replace)) {
            RagService.updatePrompt(ragConfig, ragData, replace, ragData.getQuery().getMetadata(FeatureField.KEY_PLUGIN_DIR, String.class));
        }
    }
