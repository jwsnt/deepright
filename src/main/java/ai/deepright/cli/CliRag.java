...
public class CliRag extends RagCondition implements RagService {
    ...
    protected void updateOrigin(RagConfig ragConfig, RagData ragData) throws Exception {
        // 上游标准报文头，首字母大写
        String replace = MapUtils.getString(ragConfig.getGlobalConfig(), FeatureField.KEY_ORIGIN.toLowerCase());
        if (!StringUtils.isEmpty(replace)) {
            RagService.updatePrompt(ragConfig, ragData, replace, FeatureUtils.buildOrigin(ragData.getQuery()));
        }
    }
