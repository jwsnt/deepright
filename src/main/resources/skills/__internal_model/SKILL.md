...
### 通用
+ 创建前先执行`#app api <功能> check`。仅当模型未安装或不可用时，读取绝对路径`#config`中的JSON，并将对应`<功能>.install`的完整文本作为唯一安装规范：不得猜测、简化或改写版本、来源、回退顺序、权重路径和验证要求。仅在官方源失败后使用其中指定的回退源。完成该文本规定的验证后，才能继续创建任务，并按其中要求报告结果
