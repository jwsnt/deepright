...
- 浮层内容只统计当前 origin 下已经落盘到 `localStorage` 的数据，不再混入运行时 `state.chats`、运行时 CLI 历史或其他尚未持久化的页面内存态。
- 浮层第一排会先展示总览卡片：`本地总占用`、`会话存储`、`最大会话`。
- 第二部分统一改为 `会话空间汇总` 列表，不再展开 key 明细或当前会话字段级诊断。
- 每个会话只展示汇总后的已落盘空间结构，重点用于快速排查“哪个会话占空间、主要占在哪一类”：
  - `正文`
  - `快照`
  - `原始流`
  - `元数据`
- 这里仍会明确把右侧 URL 预览归到轻量元数据；运行时 `CLI` 继续保持“只展示、不持久化”，也不会再进入这层统计。
