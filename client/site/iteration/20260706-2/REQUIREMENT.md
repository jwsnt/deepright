...
### 需求介绍
+ 首次点击清空会话按钮（无任何点击清空会话按钮记录）时开启引导对话输入的新手引导任务
    + 在清空会话按钮增加新手引导的遮罩，高亮按钮，并暗淡其他部分并不可点击
        + 新手引导文案：清空会话可以让界面保持整洁。
        + 提供一个`知道了`的新手引导按钮，切换重新加载会话按钮，并暗淡其他部分并不可点击
    + 在重新加载会话按钮增加新手引导，高亮按钮，并暗淡其他部分并不可点击
        + 新手引导文案：可以通过重新加载会话恢复内容。
