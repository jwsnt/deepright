...
### 需求介绍
+ 在设置中首次成功开启并保存蜂群配置（无任何设置中成功开启并保存蜂群配置时记录）时开启新手引导任务
    + 高亮居中对话输入框的swarm按钮，无论是否处于开启状态都模拟一个从关闭到开启的动画（2秒），并暗淡其他部分并不可点击
        + 新手引导文案：在当前对话中开启蜂群后可以自动或指定调用蜂群成员
        + 提供一个`知道了`的新手引导按钮，切换居中对话输入框，并暗淡其他部分并不可点击
    + 在居中对话输入框增加新手引导，模拟动画输入@唤起Agent菜单，选择首次成功开启并保存的蜂群Agent，并暗淡其他部分并不可点击
        + 新手引导文案：蜂群成员间只能相互调用。
        + 提供一个`完成`的新手引导按钮，点击后关闭新手引导任务的遮罩（模拟动画输入的@）去掉，所有功能可点击
    + 新手引导期间居中对话框的发送按钮不能响应键盘回车避免误发送，新手任务完成后恢复响应。
