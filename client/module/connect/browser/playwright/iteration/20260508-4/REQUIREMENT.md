...
### 需求介绍
+ 为browser_playwright代理的Playwright命令自动添加session, 而不需要通过命令行添加

### 案例参考
+ 如果没有指定--session但指定agentId和chatId则自动注入
    + 首先归一化--session参数为小写字母，防止因为大小写引起的匹配错误
``` 自动添加--session agent-a@ctrip-home
./browser_playwright --agentId agent-a --chatId ctrip-home eval 'document.body ? document.body.innerText.slice(0, 1000) : ""'
```
+ 如果指定--session但又指定agentId和chatId则使用--session
``` session还是使用agent-a@ctrip-home
