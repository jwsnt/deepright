...
### 需求介绍
+ 新增邮件CLI命令send，向邮件推送消息
```
./email send --message 原消息报文（json string） --content 消息文本内容 --image 以逗号分隔的图片附件 --file 以逗号分隔的文件附件
```
    + message为add-request的原始报文
        + add-request需求： ../../../REQUIREMENT.md
    + image、file 可为空
+ 启动参数（如密钥）等获得方式同./email start
    + 获取参数需求：../../../iteration/20260504-2/REQUIREMENT.md
+ 文本消息为发送飞书富文本、图片和文件使用邮件附件
+ 如果参数message存在则使用回复邮件，并进行跟踪，帮助发件人将回复归入同一个会话中（遵守RFC 5322）
    + In-Reply-To = 父邮件的 Message-ID，References = 原有 References + 父邮件的 Message-ID
+ 历史邮件再次作为Query回复时，必须沿用原邮件的会话链路发送，而不能被系统当成一封新的独立邮件
+ 每次推送时，在email.log记录调用
