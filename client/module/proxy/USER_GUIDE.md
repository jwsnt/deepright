...
## 迭代 20260620-1 至 20260709-1：Agent 配置与会话状态

- Agent 可保存媒体相关配置，转发聊天和 CLI 请求时会使用最新配置。
- 知识库状态按 Agent 分离，转发请求会带上当前 Agent 的知识库信息。
- 沙盒状态仅由非空 `chatId` 定位；同一会话切换 Agent 后仍保持同一沙盒模式。

---

## 迭代 20260620-1 至 20260709-1：Agent 配置与会话沙盒

- Agent 可保存媒体相关配置；转发聊天与 CLI 任务时会读取最新的 Agent 配置。
- 知识库路径、更新时间和提交状态按 Agent 独立返回，已有调用方传入的 metadata 不会被覆盖。
- 沙盒状态以非空 `chatId` 为唯一键。写入接口仍会接收 `agentId` 以便记录日志，但切换 Agent 不会改变同一会话的沙盒状态。
