...
### 需求介绍
+ 在启动浏览器插件成功后首次鼠标悬停在左侧边栏的浏览器插件上（此时浏览器插件必须为启动状态的绿色）后（无任何成功启动任意插件的记录）时开启新手引导任务
    + 在居中对话框动画模拟键盘输入@唤起技能，选择__internal_browser，高亮__internal_browser技能，并暗淡其他部分并不可点击
        + 新手引导文案：可以主动使用@__internal_browser技能唤起浏览器插件。
        + 提供一个`完成`的新手引导按钮，点击后动画到一个确认浮层，询问用户是否需要测试插件，点击则使用键盘模拟动画在自动在居中输入框选择@技能__internal_browser并输入在维基百科搜索下Deepseek，并自动发送。
            + 确认浮层需要绑定回车为是，ESC为否
            + 无论确认结果是还是否，都需要关闭新手引导任务的遮罩（收起键盘模拟），所有功能可点击
