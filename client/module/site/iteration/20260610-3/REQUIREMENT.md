...
### 需求介绍
+ 在居中输入框首次输入@唤起@文件技能菜单时（无任何@唤起@文件技能菜单时的记录）时开启新手引导任务
    + 展开@文件技能菜单增加新手引导的遮罩，高亮菜单整个区域，使用动画模拟上下按钮，在文件和技能之间切换（切换延迟2s)，并暗淡其他部分并不可点击
        + 新手引导文案：使用键盘上下选择文件还是技能。
        + 提供一个`知道了`的新手引导按钮，点击后新手引导的遮罩切换@文件或技能中的文件，高亮文件选项区域，并暗淡其他部分并不可点击
    + 模拟动画，键盘输入根目录，然后展示模拟的3个目录和2个文件，并暗淡其他部分并不可点击
        + 新手引导文案：通过键盘上下选择目录中的文件。
        + 提供一个`知道了`的新手引导按钮，点击后新手引导的遮罩切换第一个模拟的目录，需要有键盘按键右的动画，高亮整个@文件或技能区域，并暗淡其他部分并不可点击
    + 在@文件或技能区域的模拟目录区域，再模拟3个文件增加新手引导的遮罩，高亮整个@文件技能区域，并暗淡其他部分并不可点击
        + 新手引导文案：通过键盘右进入选择目录。
