...
### 需求介绍
+ 在居中输入框首次加入待处理消息列表（无任何加入待处理消息列表）时开启新手引导任务
    + 展开待处理列表，并暗淡其他部分并不可点击
        + 新手引导文案：待处理消息会在当前会话结束后自动发送。
           + 提供一个`知道了`的新手引导按钮，点击后高亮第一条待处理消息，并暗淡其他部分并不可点击
        + 在第一条待处理消息的编辑按钮处新手引导的遮罩，高亮编辑按钮按钮，并暗淡其他部分并不可点击
            + 新手引导文案：点击编辑在发送前修改内容。
            + 提供一个`知道了`的新手引导按钮，高亮删除按钮，并暗淡其他部分并不可点击
