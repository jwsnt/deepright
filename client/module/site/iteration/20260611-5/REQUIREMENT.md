...
### 需求介绍
+ 在居中对话框首次识别出文件或图片URL时（无论是用户输入还是SSE响应）（无任何居中对话框首次识别出图片URL时（无论是用户输入还是SSE响应）记录）时开启新手引导任务
    + 在识别出文件或图片URL的区域展开预览、下载文件、打开目录三个悬浮菜单，增加新手引导，高亮菜单和识别URL内容区域，并暗淡其他部分并不可点击
        + 新手引导文案：图片可以进行预览和编辑。
