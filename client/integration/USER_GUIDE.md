...
### Agent `skills` 与 `app` 本地 Git 备份

主应用静态 `config/config.json` 的 `backup` 对象分别控制每个 Agent 的 `skills/` 与 `app/` 本地 Git 备份周期，单位为分钟：
...
两项都必须是正整数。Integration 在启动后于后台立即各执行一次，之后分别按各自的周期执行；备份不会阻塞 HTTP 服务、聊天、定时任务或其他后台工作。`backup` 缺失、不是对象，或任一周期缺失、为零、为负数、非整数或超出 `time.Duration` 范围时，备份整体禁用并在 Integration 日志中记录配置错误。

`--agent-dir` 下的每个一级目录都会参与扫描。若存在目标目录，`<Agent>/skills/` 与 `<Agent>/app/` 各自维护独立的本地 Git 仓库（分别位于目录自己的 `.git/`）；不会添加远程仓库、拉取或推送。目标目录缺失时跳过，不会为了备份创建空目录。

备份递归提交所有符合条件的普通文件、符号链接、隐藏文件、层级和已有 `.gitignore`。源目录的 `.gitignore` 不会阻止合规文件被备份；Git 无法表示的空目录不会备份。图片、音视频、压缩包、可执行文件/动态库、字节码、Office/PDF、字体、磁盘镜像和数据库等内置二进制扩展名一律排除，扩展名比较不区分大小写；大小严格大于 `10,000,000` 字节的文件也会排除，恰好 10MB 的文件仍会提交。

当已跟踪的文件被删除、变为上述二进制扩展名或超过大小限制时，Integration 只会把它从 Git 索引移除，绝不会删除工作目录的源文件。暂存区没有实际变化时不会创建空提交；没有预设 Git 身份时，仓库会只在本地补齐提交身份，已有身份不会被覆盖。扫描、初始化、暂存或提交失败时，日志会记录 Agent、目录、仓库路径与失败原因，且不影响其他 Agent 或目录继续备份。

在虚拟文件系统进入当前 Agent 的 `skills/` 或 `app/` 根目录时，界面会显示该目录“最后备份”的本地时间。时间只来自此目录独立仓库中最近一条 `deepright backup` 提交，因此用户手动创建的普通 Git 提交不会被误报为备份；还没有成功提交时会显示“暂无已提交备份”。该查询异步执行，失败时不会阻止目录内容加载，也不会显示不确定的时间。
...
- 前向恢复会在原有 `chat_log` 结果之外，继续尝试合并同一 `agentId + chatId` 下的 CLI 事件日志，把 `cli/get`、`cli/pub` 一并返回给前端；`history=1` 冷历史只读取并返回中心正文所需的 `Q`、`A`、`X`
...
Integration 新增本机接口 `POST /api/model/test`，供设置页测试尚未保存的单行模型配置。接口只读取运行时 `config/config.json.test.content` 和 `config/config.json.test.timeout`，并以 UUID、`metadata.test = true` 的隔离 SSE 请求调用服务商。
...
- `skills_git_install` 缺失、为空或不含 `$git_path`，以及配置读取失败时，页面会在浮层显示错误且不会发送请求。

详细说明见 [iteration/20260718-2/USER_GUIDE.md](iteration/20260718-2/USER_GUIDE.md)。

---

## 迭代 20260620-1 至 20260718-1：会话与运行维护

- 每个 Agent 可拥有独立知识库和静态应用资源；管理员可通过复制接口迁移 Agent 的必要工作区与知识库内容。
- 会话可按 `chatId` 禁用技能或切换沙盒，恢复时会同时带回按时间排序的 CLI 子任务历史。
- 多个同源窗口的会话列表会通过本地变更、定时补拉和回到前台刷新保持最终一致。
- 自动任务会使用当前模型配置；设备配置支持热更新。日志会定期清理过期数据。
- Integration 启动后会确定唯一服务端口，并在转发请求 metadata 中携带该端口；插件目录也会一并传递给上游。

---

## 迭代 20260620-1 至 20260718-1：资源、会话与运行维护

- 每个 Agent 可通过 `/mapping/$agentId/*` 提供自己的 `app` 静态资源，并可用 `/api/copy` 复制必要工作区与知识库内容。
- 会话支持插入消息确认、按会话禁用技能、恢复 CLI 事件历史，以及按 `chatId` 共享沙盒状态。
...
## 迭代 20260726-1：预览媒体多坐标提取

Site 的视频和图片预览均可提取一个或多个坐标。该交互完全在浏览器本地完成，不新增 Integration 接口，也不会上传、修改或转换媒体文件。

- 点击坐标图标后，在预览浮层中央输入提取数量；默认 `1`，`Enter` 确认，`Esc` 取消。
- 坐标以视频原始帧或图片原始 Canvas 的左上角为 `(0,0)`，Y 轴向下增长，可直接用于 Canvas / FFmpeg；缩放预览不影响导出的原始像素坐标。
- 多点提取期间会在画面上显示编号标记，并在右下角显示剩余次数；全部完成后才会一次性写入剪贴板。浅色主题使用 `#FF4500` 暖色标记，深色主题使用冷色标记。
- 图片开始坐标提取时会关闭切图工具；完成、取消、按 `Esc`、再次点击坐标图标或关闭预览都会清除标记和临时坐标。

完整说明见 [iteration/20260726-1/USER_GUIDE.md](iteration/20260726-1/USER_GUIDE.md)。

---

## 迭代 20260726-2：新建文件后直接编辑

在 Site 的虚拟文件系统中，选择纯文本文件（例如 `txt`、`json`、`md`、`html`）并创建成功后，页面会以本次提交的 workspace 相对路径自动打开该文件的预览浮层。

- 文本和 HTML 文件会在内容读取完成后直接展示“编辑”页签，不会先渲染文件预览内容。
- 目录创建、创建失败、请求异常，以及图片、音视频、PDF 和其它非纯文本文件的创建，均维持原有行为，不会自动打开文件预览。
- 本次不新增文件格式转换、编辑器或 HTTP 接口。

完整说明见 [iteration/20260726-2/USER_GUIDE.md](iteration/20260726-2/USER_GUIDE.md)。

---

## 迭代 20260726-3：居中浮层期间隐藏 URL 预览

右侧 URL `iframe` 预览打开时，只要页面显示居中、阻断交互的模态浮层（包括虚拟文件系统的“新建文件或目录”和“上传文件或目录”），预览会暂时隐藏。

- 关闭全部此类浮层后，预览会恢复到原来的右侧或页内展开状态；当前地址、已加载页面和会话绑定都会保留，不会重新加载或关闭网站浏览。
- 此行为不会收起右侧边栏，也不会修改用户的右侧栏偏好。
- Toast、非阻断交互的加载提示、左右侧栏锚定浮层、右侧栏内确认框和浏览器原生文件选择器不会触发隐藏。
- 页内展开的网站预览同样会在居中模态显示期间临时隐藏；此时 `Esc` 仅由当前模态浮层处理，不会还原或关闭网站预览。

完整说明见 [iteration/20260726-3/USER_GUIDE.md](iteration/20260726-3/USER_GUIDE.md)。
...
## 迭代 20260727-4：多轨 WAV 混音

`POST /api/audio_mix` 接收当前 Agent 工作区内的受限多轨工程，使用受控 FFmpeg filter graph 合成为双声道 WAV。每段音轨只能引用当前工作区内可由 FFprobe 解析的普通文件，并可声明裁剪、时间轴位置、`0.25–4×` 速率、有限循环、整体/左右声道音量、淡入淡出，以及均衡器、压缩器、混响、延迟四种效果器；任意命令、滤镜字符串、绝对路径和跨 Agent 路径都不接受。

`POST /api/audio_mix_preview` 使用相同的受限工程和校验生成临时 WAV 并以 `audio/wav` 返回，供混音浮层试听。该接口不写入或覆盖 `audios/`，在响应结束、取消或失败时清理临时文件。

- 输出名称必须是安全的单个 `.wav` 文件名，固定创建在当前 Agent 的 `audios/`。目录不存在时会创建；同名和并发冲突返回错误，绝不覆盖或自动改名。
- 采样率仅允许 `44100` / `48000`，位深仅允许 `16` / `24`，输出固定为立体声 PCM WAV。`savedAs` 返回最终绝对路径。
- 服务先生成同目录临时 WAV，再原子提交最终文件。参数、探测、依赖、转码或提交任一失败都会清理临时文件，不修改源文件，也不会留下最终 WAV。
- 该能力要求受控运行环境同时存在 `ffmpeg` 和 `ffprobe`，不会执行安装命令或提供通用转码入口。

完整说明见 [iteration/20260727-4/USER_GUIDE.md](iteration/20260727-4/USER_GUIDE.md)。

---

## 迭代 20260728-1：视频切分画面裁剪

`POST /api/video_trim` 可选接受白名单化 `crop`：`rect`、`ellipse` 或包含 3–48 个归一化点的 `polygon`。坐标必须在源画面比例范围内、形状必须有面积；补边色仅有 `black`（默认）和 `white`。未传 `crop` 时保持原有时间切分行为。

- 服务端仅从校验后的结构化几何数据生成 FFmpeg filter graph，选区内容等比缩放并居中到源画布，比例差与非矩形区域外用所选黑/白色填充；不接收滤镜、命令或客户端路径。
- 时间裁剪与画面裁剪在一次受控 H.264/AAC MP4 转码中完成；接口接受有限的实际媒体时长作为结束时间，即使该值不在前端 `0.5` 秒定位刻度上，因此整段视频可完整导出。输出保持源画布分辨率（H.264 仅会在奇数边向右/下补齐 1 像素），并为高分辨率画面提供最长 `15` 分钟的受控转码时间。文件仅另存当前 Agent 工作目录 `videos/`，同名不覆盖，失败会清理临时输出且源文件不变。

完整说明见 [iteration/20260728-1/USER_GUIDE.md](iteration/20260728-1/USER_GUIDE.md)。

---

## 迭代 20260728-2：虚拟文件系统最后修改时间

`GET /api/files` 的每个文件和目录条目现在都带有 `modifiedAt`，它是服务端文件系统最后修改时间的 Unix 毫秒时间戳。返回顺序固定为目录按名称升序、文件按最后修改时间降序（同一时间按名称升序）。原有的名称、类型、技能目录状态字段和按文件名前缀查询均不变。

Site 使用此字段显示目录和文件的排序，以及最近创建或修改的文件提示；Integration 不保存浏览器侧高亮、已悬停或已点击的状态。目录读取失败等情况仍沿用既有错误响应。

完整说明见 [iteration/20260728-2/USER_GUIDE.md](iteration/20260728-2/USER_GUIDE.md)。

---

## 迭代 20260728-3：视频音频剪辑

`POST /api/video_audio_edit` 接收当前 Agent 工作区的视频相对路径及结构化多轨工程，保留源视频画面，使用受控 FFmpeg graph 合成新的 AAC 音轨并封装新 MP4。工程音轨支持原视频第一音轨（可选）或工作区内新增音频，均可声明裁剪、时间轴位置、`0.25–4×` 速率、有限循环、整体/左右声道音量、淡入淡出和四种白名单效果器；接口不接受命令、滤镜字符串、绝对路径或输出目录。

`POST /api/video_audio_extract_to_audio` 仅接受当前 Agent 工作区内可预览视频的相对路径，并固定将第一音轨提取为 MP3 到 `audios/`。它不接受文件名、输出目录或 FFmpeg 参数；同名和并发竞争自动追加高精度时间戳，不覆盖已有文件或源视频。无音轨、FFmpeg/FFprobe 缺失或提取失败时不创建输出文件。

- 默认输出为 `videos/<源文件名>_edit.mp4`。调用方可提供安全的单个 `.mp4` 文件名；目录已有同名文件或发生并发竞争时，服务端自动在扩展名前追加高精度时间戳，响应以 `renamed` 标识改名，并始终不覆盖已有文件。
- 无原始音轨的视频可以仅使用新增音频导出。若最终没有可用音轨，或轨道范围超过视频时长，服务端返回错误而不创建输出。
- `POST /api/video_audio_edit_preview` 使用相同受限工程生成临时立体声 WAV 并以 `audio/wav` 返回，供前端与视频同步试听。接口不创建 `videos/` 内容；响应结束、取消或失败时清理临时文件。
...
这五类任务的日志会逐次记录任务目的、实际使用的 Python/CPython 解释器和执行入口/脚本、服务端解析的输入绝对路径、最终输出绝对路径、实际模型/权重和具体参数。下载模型或运行时资源时会先检查 HTTP Range 并使用多线程分段断点续传，不支持时回退普通下载，主源失败后继续尝试备用源；日志会记录检查、回退、源切换、断点恢复、下载源、准备写入的绝对路径、进度及校验结果。失败结束时会记录具体原因和详细错误输出。
