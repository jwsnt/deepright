...
public class RequestCheckRag extends RagCondition implements RagService {
    ...
    protected void checkOriginalQuery(RagConfig ragConfig, RagData ragData) throws Exception {
        ...
        if (StringUtils.containsIgnoreCase(MultiSourceNotifier.MAIN, SplitUtils.join(ragData.getQuery())) && !FeatureFlag.isTask(ragData.getQuery()) && !FeatureFlag.isDaemon(ragData.getQuery())) {
            WorkflowException.checkCondition(!(StringUtils.containsIgnoreCase(ragData.getQuery().getQuery(), ragData.getQuery().getOriginal())), "The request query={" + ragData.getQuery().getQuery() + "} must contain original query={" + ragData.getQuery().getOriginal() + "}");
        }
    }
    ...
    protected void checkInitialQuery(RagConfig ragConfig, RagData ragData) throws Exception {
        ...
        if (StringUtils.containsIgnoreCase(MultiSourceNotifier.MAIN, SplitUtils.join(ragData.getQuery())) && FeatureFlag.isTask(ragData.getQuery()) || FeatureFlag.isDaemon(ragData.getQuery())) {
            WorkflowException.checkCondition(!(StringUtils.containsIgnoreCase(ragData.getQuery().getQuery(), ragData.getQuery().getInitial())), "The request query={" + ragData.getQuery().getQuery() + "} must contain initial query={" + ragData.getQuery().getInitial() + "}");
        }
    }
    ...
    protected void checkSchemaOnConfig(RagConfig ragConfig, RagData ragData) throws Exception {
        ...
        if (MapUtils.getObject(ragConfig.getLlmConfig().getAdditional(), ProviderRequestService.KEY_RESPONSE_SCHEMA) != null && ragData.getRequest().isApi(ProviderRequest.REQUEST_OPENAI)) {
            WorkflowException.checkCondition(!(StringUtils.containsIgnoreCase(ragData.getPrompt(), "json")), "The request " + SplitUtils.join(ragData.getQuery()) + " must contain the `json` keyword when `rag_schema` is used: " + ragData.getPrompt());
        }
    }
    ...
    protected void checkSchemaOnRuntime(RagConfig ragConfig, RagData ragData) throws Exception {
        ...
        if (SplitUtils.equals(ragData.getQuery(), MultiSourceNotifier.MAIN) && FeatureFlag.isTask(ragData.getQuery())) {
            WorkflowException.checkCondition(!(StringUtils.containsIgnoreCase(ragData.getPrompt(), "json")), "The request " + SplitUtils.join(ragData.getQuery()) + " must contain the `json` keyword: " + ragData.getQuery().getQuery());
        }
    }
    ...
    protected void checkFunCallPlaceholder(RagConfig ragConfig, RagData ragData) throws Exception {
        ...
        if (!CollectionUtils.isEmpty(providerFunCallL)) {
            for (ProviderFunCall funCall : providerFunCallL) {
                WorkflowException.checkCondition(!(TemplateChecker.check(JsonUtils.write(funCall))), "The request " + SplitUtils.join(ragData.getQuery()) + "'s funcall must not contain placeholders: " + funCall.getName());
            }
        }
    }
    ...
    protected void checkPromptPlaceholder(RagConfig ragConfig, RagData ragData) throws Exception {
        // 占位符未替换检查
        WorkflowException.checkCondition(!(TemplateChecker.check(ragData.getQuery().getQuery())), "The request " + SplitUtils.join(ragData.getQuery()) + "'s query must not contain placeholders: " + ragData.getQuery().getQuery());
        WorkflowException.checkCondition(!(TemplateChecker.check(ragData.getPrompt())), "The request " + SplitUtils.join(ragData.getQuery()) + "'s prompt must not contain placeholders: " + ragData.getPrompt());
    }
