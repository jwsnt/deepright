...
### 数据库数据
+ 复杂任务或历史数据查询可使用数据库（Sqlite：#dir/data）
+ 除非用户明确要求，只读不可写
| 表名 | 作用 |
|---|---|
| `task_meta` | 定时任务的持久化配置：周期、首次执行时间、Cron 表达式、Agent、会话、模型、任务内容及响应Schema |
| `task_detail` | 从元数据生成的实际执行明细，包含执行时间、执行结果/回复时间，以及 `started` 状态（`0` 待执行、`1` 已启动、`2` 无需启动、`3` 已完成）|
| `cron_meta_log` | 任务元数据的创建、删除等配置变更审计记录 |
| `cron_detail_log` | 任务明细的创建、启动、完成、重置等状态变化审计记录 |
