...
### 通用
+ 创建前先执行`#app api <功能> check`。仅当该命令成功返回且响应中`available: false`时，才读取绝对路径`#config`中的JSON，并将对应`<功能>.install`的完整文本作为唯一安装规范。若`check`命令报错、超时、响应格式无效或缺少`available`，不得将其视为未安装或执行安装，应报告检查失败原因。安装时不得猜测、简化或改写版本、来源、回退顺序、权重路径和验证要求。仅在官方源失败后使用其中指定的回退源。完成该文本规定的验证后，才能继续创建任务，并按其中要求报告结果
...
```
+ 如果模型未安装，则使用`#config`中`voxcpm.install`属性要求的规格下载安装
...
```
+ 如果模型未安装，则使用`#config`中`whisper.install`属性要求的规格下载安装
...
```
+ 如果模型未安装，则使用`#config`中`rembg.install`属性要求的规格下载安装
...
```
+ 如果模型未安装，则使用`#config`中`rvm.install`属性要求的规格下载安装
