...
description: 通过受管SSH会话连接用户指定的远程主机，执行授权命令并完成受控的文件上传或下载
---
...
### 超时与故障处理
+ `--timeout` 的单位为毫秒，且优先级高于 `config/config.json` 中的 `remote` 配置
+ 未指定时，`exec`与`scp`分别读取 `remote.exec_timeout`、`remote.scp_timeout`（单位为秒）；字段缺失或无效时默认超时为300秒
