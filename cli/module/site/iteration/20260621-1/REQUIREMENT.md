...
### 需求介绍
+ 在设置中首次悬停或点击多模态配置按钮（无任何首次悬停或点击多模态配置按钮记录）时开启新手引导任务
    + 在多模态配置按钮增加新手引导，高亮按钮，并暗淡其他部分并不可点击
        + 新手引导文案：编辑多模态输出参数。
        + 提供一个`知道了`的新手引导按钮，展开多模态输出参数浮层，高亮浮层，并暗淡其他部分并不可点击
    + 在多模态输出参数浮层，高亮参数区域，并暗淡其他部分并不可点击
        + 新手引导文案：参考不同模型服务商配置。
        + 提供一个`知道了`的新手引导按钮，高亮“仅作用于当前 Agent”区域元，并暗淡其他部分并不可点击
    + 在“仅作用于当前 Agent”增加新手引导，高亮参数区域，并暗淡其他部分并不可点击
        + 新手引导文案：不同Agent可以配置独立的多模态输出参数。
        + 提供一个`完成`的新手引导按钮，点击后关闭新手引导任务的遮罩（同时关闭多模态参数浮岑），所有功能可点击
