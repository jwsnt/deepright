...
### 需求介绍
+ 在虚拟文件系统首次提醒Skills文件错误时（红色闪动）（无任何提醒Skills文件错误的记录）时开启新手引导任务
    + 在被提醒Skills文件路径增加新手引导，高亮该文件，并暗淡其他部分并不可点击
        + 新手引导文案：SKILL.md存在错误，需要进行修复。
        + 提供一个`知道了`的新手引导按钮，点击后逐层动画模拟点击文件路径直到SKILL.md，新手引导的遮罩切换到SKILL.md，高亮文件，并暗淡其他部分并不可点击
    + 在引发错误提醒的SKILL.md增加新手引导，高亮该备忘元数据，并暗淡其他部分并不可点击
        + 新手引导文案：参考[https://agentskills.io/specification]修正错误，避免正确加载。
...
+ 首次新建周期（非一次性）备忘录任务成功后（无任何新建周期（非一次性）备忘录任务成功的记录）时开启新手引导任务相互独立，并且需要兼容
    + 首次新建周期（非一次性）备忘录任务需求：../20260610-5/REQUIREMENT.md
