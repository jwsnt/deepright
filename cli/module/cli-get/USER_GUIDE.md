...
1. Master 线程发送心跳请求，携带 Agent 元数据
2. 如果响应为 `HTTP 200` 且无任务，Master 立即进入下一次心跳
3. 如果有任务，交给 Worker 线程池执行，Master 立即进入下一次心跳
4. Worker 执行 Shell 命令，将结果 GZIP+Base64 编码后回传
5. 如果心跳请求失败、HTTP 非 200，或响应解析异常，则按 `--sleep` 等待后重试
