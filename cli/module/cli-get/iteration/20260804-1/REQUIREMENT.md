...
+ 独立 `cli-get` 是独立进程，无法读取 Integration 进程内的 SSE 原子计数；因此其待机策略固定为：
    + `/cli/get` 成功返回任务时，任务先进入现有本地 `taskQueue`，不等待执行或 `/cli/pub`；随后等待 `get.await` 再进行下一次请求。
    + `/cli/get` 成功返回无任务时，等待 `get.await` 再进行下一次请求。
