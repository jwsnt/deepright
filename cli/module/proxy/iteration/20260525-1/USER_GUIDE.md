...
本次迭代把 `proxy` 启动时的空 `--agent-dir` 自动补齐行为，收口为和 `/api/agent/init` 一致的默认模板初始化流程。

## 行为说明

- 启动 `proxy` 服务时，如果 `--agent-dir` 指向空目录（包括刚自动创建完成的场景），会自动创建 `DEF_AGENT/`
...
1. 启动 `proxy serve --agent-dir ./agent --default-dir ./config`，并确保 `./agent` 启动前为空目录，确认启动后 `agent/DEF_AGENT` 内包含 `config/` 下的默认模板文件。
