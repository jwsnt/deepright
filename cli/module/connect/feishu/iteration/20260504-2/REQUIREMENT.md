### 第一性原则
+ 仅可以新增/更新/删除feishu（../..）同目录及其子目录下`的文件和文件夹

### 技术规范
+ 严格遵守整体设计文档：../../../../DESIGN.md
+ 本模块设计文档：../../../DESIGN.md

### 迭代要求
+ Connect介绍：../../../REQUIREMENT.md
+ Connect手册：../../../USER_GUIDE.md
+ 不能破坏现有设计和功能

### 同步代码
+ ../../../../integration/REQUIREMENT.md
+ 所以设计/编译都需要遵守integration的二进制和CLI收口原则

### 重要提示
+ 设计前需要仔细阅读Connect的设计，需要兼容方案
    + Connect介绍：../../../REQUIREMENT.md
+ 严格遵守原始报文JSON SCHEMA：../../REQUEST_SCHEMA.json）
+ 严格遵守测试必过集：../../TEST_CASE.md

### 同步代码
+ ../../../feishu/REQUIREMENT.md
+ 所以设计/编译都需要遵守feishu的二进制和CLI收口原则

### 需求介绍
+ 新增飞书CLI命令param，返回启动飞书时需要提供的参数，是最终通过 `./integration connect meta-create` 配置飞书时 `--meta` 参数的SCHEMA
    + 飞书需求：../20260504-1/REQUIREMENT.md
    + 即使没有启动./feishu start，该命令也需要返回固定值
    + 最终使用时需要保持integration的二进制和CLI收口原则
    ```
    ./feishu param
    ```
    ``` 固定返回
    ["appId","appSecret"]
    ```
+ 新增飞书CLI命令name，返回启动飞书时需要提供的key和展示名
    + 即使没有启动./feishu start，该命令也需要返回固定值
    ```
    ./feishu name
    ```
    ``` 固定返回
    {"key":"feishu","name":"飞书"}
    ```
    + 插件标识统一原则：展示名（name）可以是中文，系统主键（key）必须稳定唯一，所有运行时链路只能用主键，不能混用展示名

### 编写代码
+ 以Golang编写以上代码，要求：
    + 编译应用名：feishu
    + 能用开源包的就用开源包
    + 代码简洁，包体积越小越好
+ 作为其他模块可以调用的子模块和可独立运行的CLI命令来编写

### 验证测试
+ 执行./feishu param，固定返回["appId","appSecret"]
+ 执行./feishu name，固定返回{"key":"feishu","name":"飞书"}
+ 使用integration顶层CLI验证参数传递关系：
```bash
./integration connect meta-create --name feishu --meta '{"appId":"x","appSecret":"y"}' --callback ./feishu --agent a --model deepseek
```
    + 其中 `--meta` 的字段集合必须与 `./feishu param` 返回值一致

### 撰写手册
+ 编写USER_GUIDE.md

### 其他要求
+ REQUIREMENT.md为需求文档，禁止编写
+ 复制至Plugin：../../../../plugins/


