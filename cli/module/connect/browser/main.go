...
func printHelp(w io.Writer) {
	...
	fmt.Fprintln(w, "  browser instance <create|init|restart|stop|shutdown|list|get> [flags]")
	fmt.Fprintln(w, "")
	...
	fmt.Fprintln(w, "  - browser instance init recreates one headed Chrome-backed CDP endpoint, registers it in instance list/get, and returns once ready")
	fmt.Fprintln(w, "  - browser instance restart/stop/shutdown manage one AgentId + ChatId Chrome instance directly")
	...
	fmt.Fprintln(w, "  ./browser instance init --agentId agent-a --chatId chat-001")
	fmt.Fprintln(w, "  ./browser instance restart --agentId agent-a --chatId chat-001")
	fmt.Fprintln(w, "  ./browser instance stop --agentId agent-a --chatId chat-001")
...
func printDaemonHelp(w io.Writer) {
	...
	fmt.Fprintln(w, "  browser daemon start [flags]")
	fmt.Fprintln(w, "  browser daemon stop [flags]")
	fmt.Fprintln(w, "  browser daemon serve [flags]")
	fmt.Fprintln(w, "")
	fmt.Fprintln(w, "Notes:")
	fmt.Fprintln(w, "  - daemon commands proxy browser_playwright start/stop/serve")
	fmt.Fprintln(w, "  - use top-level browser start/stop for plugin lifecycle reset")
}
...
func printInstanceHelp(w io.Writer) {
	...
	fmt.Fprintln(w, "  browser instance init --agentId AGENT --chatId CHAT")
	fmt.Fprintln(w, "  browser instance restart --agentId AGENT --chatId CHAT")
	fmt.Fprintln(w, "  browser instance stop --agentId AGENT --chatId CHAT")
	...
	fmt.Fprintln(w, "  - init requires a successful browser start and uses only its recorded integration config/config.json")
	fmt.Fprintln(w, "  - init validates browser.init_timeout before destroying the old AgentId + ChatId instance")
	fmt.Fprintln(w, "  - init always launches headed Chrome, registers the new instance in get/list, and returns once ready")
	fmt.Fprintln(w, "  - browser.init_timeout is a positive integer number of seconds; a missing field defaults to 300, while a missing/invalid config makes init fail")
	fmt.Fprintln(w, "  - the init timeout covers old-instance shutdown, profile setup/copy, Chrome launch, and CDP readiness; a timeout cleans the new process and state but preserves the profile")
	fmt.Fprintln(w, "  - on WSL, create/init call browser_launcher.sh beside the plugin; profileDir still stays in the normal CLI response, and its value comes from the launcher-returned user-data-dir")
	...
	fmt.Fprintln(w, "  - restart stops one managed instance and recreates it with a fresh managed port")
	fmt.Fprintln(w, "  - stop force-terminates one managed instance process and removes its saved state")
