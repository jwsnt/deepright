...
func UpsertPending(db *sql.DB, agentID, chatID, mid, message string, now time.Time) (Item, error) {
	...
	return Get(db, chatID, mid)
}

func Cancel(db *sql.DB, chatID, mid string, now time.Time) (bool, error) {
	chatID = strings.TrimSpace(chatID)
	mid = strings.TrimSpace(mid)
	if chatID == "" {
	...
	if mid == "" {
		return false, fmt.Errorf("mid is required")
	}
	if err := EnsureSchema(db); err != nil {
		return false, err
	}
	res, err := db.Exec(`UPDATE message_insert
		...
		StatusCancelled, nowOrUTC(now), chatID, mid)
	if err != nil {
		return false, err
	}
...
func Get(db *sql.DB, chatID, mid string) (Item, error) {
	if err := EnsureSchema(db); err != nil {
		return Item{}, err
	}
	var item Item
	err := db.QueryRow(`SELECT agent_id, chat_id, mid, message, status, created_at, updated_at
		...
		strings.TrimSpace(chatID), strings.TrimSpace(mid),
...
func StatusByMIDs(db *sql.DB, chatID string, mids []string) (map[string]int, error) {
	chatID = strings.TrimSpace(chatID)
	...
	args := make([]interface{}, 0, len(mids)+1)
	args = append(args, chatID)
	for _, mid := range mids {
		args = append(args, mid)
	}
	rows, err := db.Query(`SELECT mid, status FROM message_insert
		WHERE chat_id = ? AND mid IN (`+placeholders(len(mids))+`)`, args...)
	if err != nil {
		return nil, err
	}
	defer rows.Close()

	statuses := make(map[string]int, len(mids))
	for rows.Next() {
		var mid string
		var status int
		...
		statuses[strings.TrimSpace(mid)] = status
	}
