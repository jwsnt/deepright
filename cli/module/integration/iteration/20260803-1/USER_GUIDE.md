...
五类媒体任务的每次执行日志还会记录任务目的、实际启动的 Python/CPython 解释器和执行入口/脚本、服务端确认的输入文件绝对路径、最终输出文件绝对路径、实际模型/权重及具体参数。模型或运行时资源需要下载时，会先检查 HTTP Range 支持并使用多线程分段断点续传；不支持时回退普通下载，主源失败后继续尝试备用源。每个下载线程会显示线程号、分段号、分段进度和重试次数，普通下载也会按配置重试。每一次下载尝试均受连接响应、`modelTask.timeout` 指定的读取无进度时限和总时限保护。日志会给出每次检查、回退、源切换、断点恢复、重试、下载源、准备写入的绝对路径、进度、超时和校验结果；失败的最终记录会包含具体原因及详细错误输出，便于判断输入、网络、模型或输出写入问题。

五类媒体任务共用一个模型执行队列。服务启动时读取 `config/config.json` 的 `modelTask.concurrence`，该正整数决定文字转语音、音频转写、图片主体提取、人物视频对口型和视频主体提取合计可同时执行的任务数；`modelTask.download` 决定单个运行时资源下载的分段数，`modelTask.timeout` 决定每个分段或普通下载的读取无进度超时秒数，`modelTask.retry` 决定每个分段或普通下载的额外重试次数。`retry` 可为 `0`，表示失败后不重试。修改任一值后重启 Integration 生效；缺失或非法值使用安全默认值并记录服务日志。
