...
### 需求介绍
+ 调整integration内部会话沙盒状态维度：从`agentId + chatId`改为仅按`chatId`存储和命中
...
+ integration内部通过共享sqlite读取/写入`cli_sandbox_state`时，不允许再以`agentId`作为主定位条件
    + 共享状态结构需要与`cli-get`保持一致
    + 旧数据不需要迁移，允许直接按新结构重建
+ `/api/cmd`和integration内部任务执行链路的沙盒判断需要仅按`chatId`命中沙盒模式
