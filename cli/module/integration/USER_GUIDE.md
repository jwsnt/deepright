...
`POST /api/video_audio_extract` 接收当前 Agent 工作区内可预览视频的 `agentId` 和相对 `path`，只提取第一个音轨并在该工作区的 `videos/` 目录创建 MP3。接口不接受输出路径、文件名、FFmpeg 参数或滤镜；源视频始终保持不变。

- 输出默认沿用源视频文件名并替换为 `.mp3`。若 `videos/<同名>.mp3` 已存在，服务会在扩展名前自动追加高精度时间戳；并发冲突也会自动重试，绝不覆盖已有文件。
- 服务端先使用 FFprobe 检测音轨。没有可提取音轨时返回可展示的错误，不创建目录、临时文件或最终 MP3；FFmpeg/FFprobe 缺失、探测失败、转码失败或超时也不会留下最终文件。
- 转换在受限工作区内使用固定 FFmpeg 参数先生成临时 MP3，再原子创建最终文件。成功响应的 `savedAs` 是最终绝对路径。
