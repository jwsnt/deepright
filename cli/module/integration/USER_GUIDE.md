...
`POST /api/audio_mix` 接收当前 Agent 工作区内的受限多轨工程，使用受控 FFmpeg filter graph 合成为双声道 WAV。每段音轨只能引用当前工作区内可由 FFprobe 解析的普通文件，并可声明裁剪、时间轴位置、`0.25–4×` 速率、有限循环、音量、左右声道、静音、淡入淡出，以及均衡器、压缩器、混响、延迟四种效果器；任意命令、滤镜字符串、绝对路径和跨 Agent 路径都不接受。
